 ...
 */
function jjamerson_arc_preprocess_page(&$variables) {
  ...
  /* If this is a development environment, rebuild the minified JS file.
     ...
     1. PHP needs write permissions for /js/jjamerson_arc.min.js
  ...
  */
  ...
  if ( $dev_env == 1) {
    ...
    $packed = $packer->pack();
    file_put_contents( dirname(__FILE__) . '/js/jjamerson_arc.min.js', $packed);
  } elseif ( $dev_env == 2) {
...
function jjamerson_arc_process_page(&$variables) {
...
function jjamerson_arc_preprocess_region(&$region) {
  if ( isset($node['language']) ) {
    $lang = $node['language'];
  } else {
    $lang = 'und';
 ...
 */
function jjamerson_arc_preprocess_html(&$variables) {
  ...
  if ( $site_section && isset($site_section->field_base_url['und'][0]['value']) ) {
    ...
    if ( drupal_get_path_alias() === $site_section->field_base_url['und'][0]['value'] ) {
      ...
      if ( empty($site_section->field_standalone['und'][0]['value']) ) {
        $variables['head_title'] .= ' | Berklee College of Music Archives';
      }
    } else {
  ...
  if ( isset($_GET['iframed']) ) {
    ...
    drupal_add_html_head($target, 'jjamerson_arc-iframe-target');
  }
...
function jjamerson_arc_process_html(&$variables) {
...
function jjamerson_arc_process_views_view_field(&$variables) {
...
function jjamerson_arc_process_field(&$field) {
  global $user;
...
function jjamerson_arc_preprocess_node(&$node) {
  global $user; // Access to Drupal's global for currently logged-in user
 ...
 */
function jjamerson_arc_css_alter(&$css) {
 ...
 */
function jjamerson_arc_js_alter(&$js) {
  //unset($js[ path_to_theme() . '/js/jjamerson_arc.min.js' ])
  if ( isset($_GET['render-only']) || isset($_GET['content-only'])) {
    unset($js[ 'sites/all/modules/patched/admin_menu/admin_menu.js' ]);
  }
}

function jjamerson_arc_menu_link($variables) {
...
function jjamerson_arc_apachesolr_search_mlt_recommendation_block($vars) {
 ...
 *
 */
function jjamerson_arc_form_views_exposed_form_alter(&$form, &$form_state) {
